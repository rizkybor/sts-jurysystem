...
export const POST = async (request) => {
  try {
    ...
    const eventCategories = formData.getAll('eventCategories').map((id) => ({ $oid: id }));
    const divisionCategories = formData.getAll('divisionCategories').map((id) => ({ $oid: id }));
    const raceCategories = formData.getAll('raceCategories').map((id) => ({ $oid: id }));
    const initialCategories = formData.getAll('initialCategories').map((id) => ({ $oid: id }));
    ...
    return new Response(JSON.stringify({ message: 'Event created successfully', newEvent }), {
      status: 201,
    });
  } catch (error) {
    console.error(error);
